...
class GeminiProvider(AIProvider):
    ...
    def __init__(self, api_key: str):
        super().__init__(api_key)
        if api_key:
            # 최신 버전 방식: configure를 사용하여 API 키 설정
    ...
    async def generate_response(self, messages: List[Dict], system_prompt: str, temperature: float = 1.0, model: str = None, max_output_tokens: int = 2000) -> str:
        ...
        try:
            ...
            # 최신 버전 방식: GenerativeModel 객체 생성
            ...
            for msg in messages:
                ...
                if text_content:
                     gemini_messages.append({'role': role, 'parts': [text_content]})

            if not gemini_messages:
                return "❌ No valid messages to send to Gemini."

            # 최신 버전 방식: generate_content_async 사용
